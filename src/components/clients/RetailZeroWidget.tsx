...
import { RETAILZERO_AI_STEPS, CONSENT_DENIED_STEPS_A, CIBA_DENIAL_STEPS_A, computeEffectiveSteps } from "@/lib/scenario";
import { ChatSimulator } from "./ChatSimulator";
import { Button } from "@/components/ui/button";

interface RetailZeroWidgetProps {
  onAddSecurityEvent: (event: SecurityEvent) => void;
}

export function RetailZeroWidget({ onAddSecurityEvent }: RetailZeroWidgetProps) {
  ...
  const effectiveSteps = useMemo(
    () => computeEffectiveSteps(RETAILZERO_AI_STEPS, gateDecisions, CIBA_DENIAL_STEPS_A, CONSENT_DENIED_STEPS_A),
    [gateDecisions],
  );
  ...
  return (
    <div className="client-retailzero fixed bottom-6 left-6 z-30 w-[648px] max-w-[calc(100vw-3rem)] h-[580px] max-h-[calc(100vh-3rem)] rounded-2xl bg-white border shadow-2xl flex flex-col overflow-hidden animate-slide-up scrollbar-hidden">
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b bg-[#4016A0]/5 shrink-0">
        ...
        <div className="flex-1">
          <span className="text-sm font-semibold text-[#191919]">RetailZero AI</span>
      ...
      <div className="px-3 pt-2 pb-1 shrink-0">
        <div className="flex items-center gap-2 rounded-xl bg-black/[0.03] px-3 py-2.5 border">
          <span className={`flex-1 text-sm ${userTyping.typing ? "text-[#191919]" : "text-black/40"}`}>
            {userTyping.typing ? userTyping.text : "Message RetailZero AI..."}

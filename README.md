# StyleVault: Securing AI Agent Commerce with Auth0

StyleVault is an interactive demo that shows how Auth0 secures AI agents as they browse, transact, and operate on behalf of e-commerce customers. It walks through three progressively sophisticated scenarios -- a 1st-party AI widget, ChatGPT via MCP, and Gemini via UCP -- all protected by a single Auth0 security layer.

The demo tells a cohesive story: StyleVault, a premium e-commerce platform, moved from zero to secured AI commerce in weeks by using Auth0's managed OAuth infrastructure instead of building custom auth from scratch.

## What This Demo Covers

**Scenario A: 1st-Party AI Widget**
StyleVault's own embedded AI assistant connects through a simple consent flow, executes scoped tool calls (wishlist, search, purchase), and hits bounded authority limits on high-value transactions.

**Scenario B: ChatGPT via MCP (Model Context Protocol)**
A third-party AI client discovers StyleVault's MCP server, dynamically registers itself, authenticates the user, and transacts without pre-registration or manual onboarding. Human-in-the-loop approval via CIBA handles sensitive purchases.

**Scenario C: Gemini via UCP (Universal Commerce Protocol)**
An autonomous AI agent discovers StyleVault's merchant manifest, negotiates capabilities via profile exchange, and authenticates requests using HTTP Message Signatures (RFC 9421). Auth0 handles Identity Linking when the agent needs user-specific data. When a checkout exceeds the merchant's agent policy limit, UCP's state machine triggers escalation via `continue_url`, and StyleVault uses Auth0 CIBA behind that URL for buyer approval.

## Auth0 Features in Action

### Discovery and Zero-Friction Onboarding

- **Protected Resource Metadata (RFC 9728):** StyleVault publishes `/.well-known/oauth-protected-resource` so AI clients discover authorization requirements automatically.
...
- **UCP Merchant Manifest:** For autonomous agents, StyleVault publishes `/.well-known/ucp` describing commerce capabilities, endpoints, payment handlers, and authority policies.
...
- **Merchant Agent Policy (UCP):** For UCP, StyleVault enforces a server-side agent transaction limit. This is a merchant-configured policy, not a token claim. The AP2 (Agent Payments Protocol) handles payment authorization through tokenized, verifiable credentials.
...
- **Capability Negotiation:** Before any commerce occurs, both sides exchange profiles. Gemini declares what it can do, StyleVault declares what it allows, and only the intersection proceeds. Commerce capabilities are authorized through this negotiation, not OAuth scopes.
- **Agent Identity Validation:** StyleVault verifies the agent's request signature against its published `signing_keys`. Auth0 handles a separate concern: Identity Linking, which connects the agent to a specific user's account.
- **Checkout State Machine:** UCP defines explicit states (`incomplete` > `requires_escalation` > `ready_for_complete` > `completed`). When a checkout exceeds the merchant's agent policy limit, UCP returns `requires_escalation` with a `continue_url` for buyer approval. StyleVault implements buyer approval behind that URL using Auth0 CIBA.
- **Merchant Agent Policy:** StyleVault configures a $250 agent transaction limit as server-side policy. This is enforced by the merchant application, not as a token claim. The limit applies to all agent-initiated checkouts regardless of which agent platform is connected.
...
6. ChatGPT  -->  StyleVault MCP Server                   (tool calls with access token)
```
...
LAYER 1: Agent-to-Merchant Authentication (HTTP Message Signatures, RFC 9421)
  ...
  StyleVault verifies against Gemini's published signing_keys.
...
LAYER 3: Buyer Approval (continue_url + Auth0 CIBA)
  ...
  UCP returns continue_url; StyleVault implements Auth0 CIBA behind it.
```

```
1. Gemini   -->  /.well-known/ucp                        (discover merchant manifest)
2. Gemini  <->  StyleVault                               (profile exchange + capability negotiation)
3. StyleVault   verifies Gemini's request signature       (RFC 9421, agent's public key)
4. Gemini   -->  Auth0 /authorize                        (Identity Linking: user login + consent)
5. Gemini   -->  Auth0 /oauth/token                      (Identity Linking tokens)
6. Gemini   -->  StyleVault UCP Endpoints                (signed requests for commerce operations)
7. StyleVault   returns requires_escalation + continue_url (if over merchant policy limit)
8. StyleVault   -->  Auth0 /bc-authorize                 (CIBA buyer approval behind continue_url)
```
...
|-----|----------|--------|----------|----------|
| 1 | StyleVault AI + ChatGPT | 1st-party widget + ChatGPT | MCP | ~4 min |
...
| `src/components/clients/StyleVaultWidget.tsx` | 1st-party AI widget |
